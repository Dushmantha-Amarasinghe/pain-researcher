...
from pain_researcher.models import PainPointCandidate, RedditComment, RedditThread
...
class Prefilter:
    ...
    def _passes_thresholds(self, thread: RedditThread) -> bool:
        cfg = self._config
    ...
    def _has_complaint_signal(self, thread: RedditThread) -> bool:
        ...
        """
        if self.matches_complaint(thread.title) or self.matches_complaint(thread.selftext):
            return True
        return any(self.matches_complaint(c.body) for c in thread.comments)

    def _reprioritize_comments(self, thread: RedditThread) -> RedditThread:
        ...
        def relevance(c: RedditComment) -> tuple[int, int]:
